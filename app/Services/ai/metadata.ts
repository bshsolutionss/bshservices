import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "AI Automation Services | Boost Productivity & Scale with Intelligent Workflows",
  description:
    "Transform your business with AI automation. We build intelligent workflows, AI chatbots, process automation, data extraction bots, CRM automation, and custom AI tools tailored to your needs.",
  keywords: [
    "AI Automation",
    "Business Automation",
    "AI Tools",
    "AI Chatbots",
    "Workflow Automation",
    "Data Extraction Automation",
    "CRM Automation",
    "Marketing Automation",
    "Automation Services",
    "AI Agents",
    "OpenAI Automation",
    "Business Smart Hub",
  ],
  openGraph: {
    title: "AI Automation Services | Smart AI Agents for Your Business",
    description:
      "Automate tasks, reduce costs, and scale your business with custom AI solutions. From chatbots to full workflow automation — we build intelligent systems for your needs.",
    url: "/Services/ai",
    siteName: "Business Smart Hub (BSH)",
    images: [
      {
        url: "/Services/ai-banner.png",
        width: 1200,
        height: 630,
        alt: "AI Automation Services - Business Smart Hub",
      },
    ],
    locale: "en_US",
    type: "website",
  },
  twitter: {
    card: "summary_large_image",
    title: "AI Automation Services | Automate & Scale with Intelligent AI",
    description:
      "Automate workflows, boost productivity, and build smarter operations with AI-powered solutions.",
    images: ["/images/ai-automation-banner.png"],
  },
  icons: {
    icon: "/favicon.ico",
  },
};
